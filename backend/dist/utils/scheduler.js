    ...
    try {
        ...
        for (const record of upcoming) {
            ...
            if (!existingNotification) {
                await NotificationService.createNotification(record.created_by, 'Nhắc nhở bảo dưỡng xe', `Lịch bảo dưỡng định kỳ cho xe ${record.vehicles?.name || ''} (Biển số: ${record.vehicles?.license_plate || ''}) sắp diễn ra vào ngày ${record.scheduled_date.toLocaleDateString('vi-VN')}.`, 'MAINTENANCE_ALERT');
            }
        }
    }
    catch (error) {
