...
import { Prisma, payments_payment_method, payments_transaction_type } from '@prisma/client';
...
export class PaymentService {
  static async createPayment(data: {
    ...
    if (response.paymentReference || response.payUrl) {
      await PaymentRepository.updateStatus(
        payment.id,
        'PENDING',
        response.paymentReference || response.payUrl
      );
    }
  ...
  static async getPaymentById(id: bigint) {
  ...
  static async confirmPayment(id: bigint, reference?: string) {
    const payment = await PaymentRepository.findById(id);
    if (!payment) {
      throw new NotFoundError('Không tìm thấy giao dịch thanh toán');
    }
  ...
  static async refundPayment(id: bigint) {
    const payment = await PaymentRepository.findById(id);
    if (!payment) {
      throw new NotFoundError('Không tìm thấy giao dịch thanh toán');
    }
  ...
  static async updatePayment(id: bigint, data: any) {
    const payment = await PaymentRepository.findById(id);
    if (!payment) throw new NotFoundError('Không tìm thấy giao dịch thanh toán');

    const updatedData = { ...data };
    if (data.amount) updatedData.amount = new Prisma.Decimal(data.amount);
    if (data.paid_at) updatedData.paid_at = new Date(data.paid_at);

    return PaymentRepository.update(id, updatedData);
  }

  static async deletePayment(id: bigint) {
    const payment = await PaymentRepository.findById(id);
    if (!payment) throw new NotFoundError('Không tìm thấy giao dịch thanh toán');
