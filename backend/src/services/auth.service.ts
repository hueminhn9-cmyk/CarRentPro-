...
export class AuthService {
  ...
  static async register(data: { email: string; password_hash: string; full_name: string; phone?: string }) {
  ...
  static async refreshToken(oldRefreshToken: string) {
    ...
    const decodedNewRefresh: any = jwt.decode(newRefreshToken);
  ...
  static async changePassword(userId: bigint, oldPasswordCheck: string, newPasswordPlain: string): Promise<boolean> {
    const user = await prisma.users.findUnique({
      where: { id: userId },
    });
