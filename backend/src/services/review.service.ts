...
export class ReviewService {
  static async createReview(data: { booking_id: bigint; customer_id: bigint; rating: number; content?: string }) {
    // 1. Verify booking
  ...
  static async getAllReviews(options: any) {
  ...
  static async updateReview(id: bigint, customerId: bigint, data: { rating?: number; content?: string }) {
    const review = await ReviewRepository.findById(id);
    if (!review) {
      throw new NotFoundError('Không tìm thấy đánh giá');
    }
  ...
  static async getVehicleReviewStats(vehicleId: bigint) {
