...
export class BookingService {
  static async createBooking(data: {
    ...
    const bookingInput: Prisma.bookingsUncheckedCreateInput = {
  ...
  static async getBookingById(id: bigint) {
  ...
  static async getAllBookings(options: any) {
    return BookingRepository.findAll(options);
  }

  static async updateBookingStatus(id: bigint, newStatus: bookings_status, changedBy: bigint, reason?: string) {
    const booking = await BookingRepository.findById(id);
    if (!booking) {
      throw new NotFoundError('Không tìm thấy đơn đặt xe');
    }
  ...
  static async createHandover(
    ...
    data: {
      ...
      mileage: number;
      fuel_level: number;
      vehicle_condition?: 'EXCELLENT' | 'GOOD' | 'NORMAL' | 'DAMAGED';
  ...
  static async updateBooking(id: bigint, data: any, userId: bigint) {
    const booking = await BookingRepository.findById(id);
    if (!booking) throw new NotFoundError('Không tìm thấy đơn đặt xe');

    const updatedData = { ...data };
    if (data.pickup_datetime) updatedData.pickup_datetime = new Date(data.pickup_datetime);
    if (data.return_datetime) updatedData.return_datetime = new Date(data.return_datetime);
    if (data.rental_fee) updatedData.rental_fee = new Prisma.Decimal(data.rental_fee);
    if (data.service_fee) updatedData.service_fee = new Prisma.Decimal(data.service_fee);
    if (data.deposit_amount) updatedData.deposit_amount = new Prisma.Decimal(data.deposit_amount);
    if (data.total_amount) updatedData.total_amount = new Prisma.Decimal(data.total_amount);
  ...
  static async deleteBooking(id: bigint, userId: bigint) {
    const booking = await BookingRepository.findById(id);
    if (!booking) throw new NotFoundError('Không tìm thấy đơn đặt xe');
